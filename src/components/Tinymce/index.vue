...
export default {
  ...
  computed: {
    ...
    // 粘贴是否保留格式
    paste_webkit_styles() {
      if (this.mode === 'default') return 'all'
      else return 'none'
    },
    config: function() {
      // 上传配置
  ...
  methods: {
    initTinymce() {
      ...
      window.tinymce.init({
        ...
        paste_data_images: this.mode !== 'default', // 是否禁止用户粘贴图片(false为禁止)
        font_formats: `
          微软雅黑=微软雅黑;
          宋体=宋体;
          黑体=黑体;
        ...
        paste_webkit_styles: this.paste_webkit_styles,
        // 允许指定在粘贴 MS Word 和 类似Office套件产品中的内容时要保留的样式。
        paste_retain_style_properties: this.paste_webkit_styles,
        // 文件管理 相关配置字段

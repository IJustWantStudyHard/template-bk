import { login, getInfo, getCaptcha } from '@/api/definite'
...
const state = {
  ...
  // 代理商信息
  agentInfo: {},
  operator_service: null,
  market_service: null,
  is_national: 0, // 0可开启全国活动 1不可开启全国活动
  is_open_authorization: 0, // 0没有开启吸粉功能 1已经开启吸粉功能
  is_authorization: 0, // 用户是否授权成功 0没有 1已授权
  is_open_card: 0// 0没有开启卡劵功能 1已经开启卡劵功能
}
  ...
  getInfo({ commit, state }) {
    return new Promise((resolve, reject) => {
      getInfo().then(response => {
        ...
        // 修改上传文件地址
        store.commit('config/SET_UPLOAD', data.upload)
